...
//score = 736400 这是加入了ref_param之后的成绩
...
map<int, vector<string>> getTimestampPossibleBig(){
    ...
    while(not_usable_site.size() < site_bandwidth.size()){
        ...
        if(this_round_site != ""){
            ...
            //如果说这个时间步已经有了一个/多个站点作为大站点，那么查看这个新站点和已有站点的共同客户关系
            if(result.find(this_round_ts) != result.end()){
                ...
                if(can_add){
                    ...
                    if(site_skip_ts.find(this_round_site) != site_skip_ts.end()){
                        site_skip_ts[this_round_site].push_back(this_round_ts);
                    }
                    else{
                        vector<int> tmp;
                        tmp.push_back(this_round_ts);
                        site_skip_ts[this_round_site] = tmp;
                    }
    ...
    /*
    //先不用这个，好像并没有变好，单就初始解而言，把这个注释掉貌似更好一点。
    //那么再看看每个站点已经接收的大流量数目,如果没满，给它填满
    for(auto si = site_bandwidth.begin(); si != site_bandwidth.end(); si++){
        if(site_accepted_big_number[si->first] < floor(timestamps * 0.05)){
            for(auto ts_iter = site_skip_ts[si->first].begin(); ts_iter != site_skip_ts[si->first].end(); ts_iter++){
                int ts = *ts_iter;
                //因为是被跳过的站点，所以所在时间步一定已经有了站点
                result[ts].push_back(si->first);
                site_accepted_big_number[si->first] += 1;
                cout<<"add skip site = "<<si->first<<" at timestamp = "<< ts;
                cout<<" current ts chosen big site = ";
                for(auto tmp_iter = result[ts].begin(); tmp_iter != result[ts].end(); tmp_iter++){
                    cout<<" "<< *tmp_iter<<" ";
                }
                cout<<endl;
                if(site_accepted_big_number[si->first] >= floor(timestamps * 0.05)){
...
int main(){
    ...
    for(int t = 0; t < timestamps; t++){
        ...
        //对于这个时间步的所有站点，如果站点这个时间步不是5%后时间步，那么让它贪心到V
        for(auto si = site_bandwidth.begin(); si != site_bandwidth.end(); si++){
            string site = si->first;
            //查看这个站点当前时间步有没有被分配流，有则说明它是前面被分配的大流量时间步
            if(site_t_client[site].size() > 0)
                continue;
            //给它这个时间步塞满V
            bool reach_V = false;
            for(auto cl = client4site[site].begin(); cl != client4site[site].end(); cl++){
                ...
                sort(stream_list.begin(), stream_list.end(), cmp);
                for(auto st_iter = stream_list.begin(); st_iter != stream_list.end();){
                    if(site_remaining[site] >= base_cost){
                        reach_V = true;
                        break;
                    }
                    string stream_type = (*st_iter).first;
                    int stream_bw = (*st_iter).second;
                    if(stream_bw <= site_remaining[site] && site_bandwidth[site] - site_remaining[site] + stream_bw <= base_cost){
                        st_iter = stream_list.erase(st_iter);
                        site_remaining[site] -= stream_bw;
                        if(site_t_client.find(site) != site_t_client.end() && site_t_client[site].find(*cl) != site_t_client[site].end()){
                            site_t_client[site][*cl].push_back(make_pair(stream_type,stream_bw));
                        }
                        else{
                            vector<pair<string,int>> tmp;
                            tmp.push_back(make_pair(stream_type,stream_bw));
                            site_t_client[site][*cl] = tmp;
                        }
                    }     
                    else{
                        st_iter++;
        ...
        for(auto co = client_order.begin(); co != client_order.end(); co++){
            ...
            for(auto remain_iter = client_remaining[client].begin(); remain_iter != client_remaining[client].end();){
                //对于每个流，都遍历一遍服务器，找一个最“合适”的服务器
                stream_type = remain_iter->first;
                stream_bw = remain_iter->second;
                
                string suitable_site = "";
                //现在找的是一个site当前已使用流量最少的服务器
                int site_already_in = 1410065407;
                //找剩余流量最少的服务器?????这个反而比找剩余流量多的服务器要好????大流量会堆在少部分服务器身上
                //int site_left = 1410065407;
                //找剩余流量最多的服务器
                //int site_left = -1;
                for(auto sit = actual_site.begin(); sit != actual_site.end(); sit++){
                    //if(site_remaining[*sit] >= stream_bw && site_remaining[*sit] < site_left){
                    if(site_remaining[*sit] >= stream_bw && site_bandwidth[*sit] - site_remaining[*sit] < site_already_in){
                        suitable_site = *sit;
                        //site_left = site_remaining[*sit];
                        site_already_in = site_bandwidth[*sit] - site_remaining[*sit];
                    }
                }
                if(suitable_site == ""){
                    cout<<"ERROR"<<endl;
                    ...
                    site_remaining[suitable_site] -= stream_bw;
                    site_t_client[suitable_site][client].push_back(make_pair(stream_type, stream_bw));
                }
            }
            /*
    ...
    endtime = clock();
    cout<<"time = "<<(double)(endtime-start)/CLOCKS_PER_SEC<<endl;
