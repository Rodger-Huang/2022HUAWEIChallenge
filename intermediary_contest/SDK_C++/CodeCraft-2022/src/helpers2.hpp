//和heplers.hpp不同的地方是改了demand的结构，原来的pair改为map（因为考虑到每个时间步每个客户对每种流只有一个需求）
#include<iostream>
#include<fstream>
#include<string>
#include<sstream>
...
//const string output_path = "/output/solution.txt";
//将客户需求展开
struct UnrollClientDemand{
    string client_name;
    string stream_type;
    int stream_bw;
    int t;
};
bool compareUnrollClientDemand(UnrollClientDemand a, UnrollClientDemand b){
    return (a.stream_bw > b.stream_bw);
}
...
map<string, vector<map<string,int>>> getDemand(){
    map<string, vector<map<string,int>>> result;
    ...
    while(getline(readheader, client_name, ',')){
        ...
        vector<map<string,int>> ts_demand;
    ...
    while(getline(fdemand,demand_line)){
        ...
        if(current_time_str != time_str){ //时间步跳转
            ...
            while(getline(demand_data, detail_demand, ',')){
                map<string,int> current_ts_demand_list;
                current_ts_demand_list[stream_type] =  atoi(detail_demand.c_str());
            ...
            while(getline(demand_data, detail_demand, ',')){
                ...
                result[client_name_list[i++]][timestamps-1][stream_type] = atoi(detail_demand.c_str());
            }
        }
    }
    fdemand.close();
    if(CHECK_DATA == 2){
        ...
        for(auto it = result.begin(); it != result.end(); it++){
            ...
            for(int i = 0; i < it->second.size(); i++){
                ...
                map<string, int> tmp = it->second[i];
