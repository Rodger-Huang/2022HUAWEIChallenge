...
bool cmp2(pair<string, pair<double, int>> a, pair<string, pair<double, int>> b){
    // 优先比较后面pair中的第一个元素
...
int main(){
    ...
    for(int t = 0; t < timestamps; t++){
        ...
        for(auto co = client_order.begin(); co != client_order.end(); co++){ // 先处理可用边缘节点多的客户
            ...
            priority_queue<pair<string, double>, vector<pair<string, double>>, queue_cmp> site_order;
            double value;
            for(auto si = actual_site.begin(); si != actual_site.end(); si++){
                // if(site_t_usage[*si][t] > base_cost){
                //     value = 1 + pow((site_t_usage[*si][t] - base_cost), 2) / site_bandwidth[*si] + site_t_usage[*si][t];
                // }else if(site_t_usage[*si][t] > 0){
                //     value = (double) site_t_usage[*si][t] / base_cost;
                // }else{
                //     value = 1;
                // }
                // pair<string, double> tmp = make_pair(*si, value);
                // site_order.push(tmp);
                site_order.push(make_pair(*si, 0));
            }
            // 这里暂且使用擦除vector中元素的做法,如果出错,可替换成加一个flag进行判断,去计算vector中每个元素是否为0
            // 对客户的不同流需求进行排序
            sort(client_remaining[client].begin(), client_remaining[client].end(), cmp);
            ...
            while(remain_iter != client_remaining[client].end()){
                // 找到一个能用的site
                // 找当前分配流量最小的节点
                string site = site_order.top().first;
                value = site_order.top().second;

                vector<pair<string, int>> to_be_reserved;
                while(site_remaining[site] < stream_bw){
                    to_be_reserved.push_back(make_pair(site, value));
                    site_order.pop();
                    site = site_order.top().first;
                    value = site_order.top().second;
                }
                remain_iter = client_remaining[client].erase(remain_iter);
                site_remaining[site] -= stream_bw;
                site_t_usage[site][t] += stream_bw;
                site_order.pop();
                // if(site_t_usage[site][t] > base_cost){
                //     value = 1 + pow((site_t_usage[site][t] - base_cost), 2) / site_bandwidth[site] + site_t_usage[site][t];
                // }else{
                //     value = (double) site_t_usage[site][t] / base_cost;
                // }
                // site_order.push(make_pair(site, value));
                site_order.push(make_pair(site, value + stream_bw));
                ...
                for(auto it = to_be_reserved.begin(); it != to_be_reserved.end(); it++){
                    site_order.push(*it);
                }
            }
        }
    ...
    int position_90 = int(ceil(timestamps * 0.90) - 1);
    set<string> site_90;
    // priority pull
    for(int i = 0; i < 2; i++){
        // site_90.clear();
        ...
        vector<pair<string, pair<double, int>>> site_order_95_and_client;
        vector<pair<string, int>> site_order;
        // 记录每个边缘节点大于V值的时刻
        ...
        for(auto it = site_bandwidth.begin(); it != site_bandwidth.end(); it++){
            ...
            if(value_95 > base_cost){
                value = 1 + pow((value_95 - base_cost), 2) / site_bandwidth[site] + value_95;
            }else if(value_95 > 0){
                value = (double) value_95 / base_cost;
            }else{
                value = 1;
            }
            // value = (double)(site_bandwidth[site] - value_95) / site_bandwidth[site];
            if(client4site.find(site) != client4site.end()){
                site_order_95_and_client.push_back(make_pair(site, make_pair(value, client4site[site].size())));
                site_order.push_back(make_pair(site, client4site[site].size()));
            }
            else{
                site_order_95_and_client.push_back(make_pair(site, make_pair(value, 0)));
                site_order.push_back(make_pair(site, 0));
            }
            // for(int t = 0; t < timestamps; t++){
            //     if(site_t_usage[site][t] > base_cost){
        ...
        int num_site = site_order_95_and_client.size();
        // 选择成本代价最大的10个为90%节点
        if(i == 0){
            for(int i = 0; i < 10; i++){
                site_90.insert(site_order_95_and_client[i].first);
            }
        }
        // if(i == 0){
        //     for(int i = 0; i < 10; i++){
        //         site_90.insert(site_order[i].first);
        //     }
        // }
        if(i == 1){
            reverse(site_order_95_and_client.begin(), site_order_95_and_client.end());
        }
        for(auto si = site_order_95_and_client.rbegin(); si != site_order_95_and_client.rend(); si++){
            string site = si->first; // 当前处理的边缘节点
            if(i == 1){
                if(site_90.find(site) == site_90.end()){
                    continue;
                }
            }
            ...
            int value = 0;
            if(site_90.find(site) != site_90.end()){
                value = site_t_usage[site][index[position_90]];
            }else{
                value = site_t_usage[site][index[position_95]];
            }
            site_more_95[site] = value;
            si->second.first = value;
            if(value <= base_cost){
                value = base_cost;
            }
            ...
            for(int position = 0; position < int(index.size()); position++){
                ...
                int position_tmp;
                if(site_90.find(site) != site_90.end()){
                    position_tmp = position_90;
                }else{
                    position_tmp = position_95;
                }
                if(position <= position_tmp){
                    left = value - site_t_usage[site][t];
                }
                else{
    ...
    if(CHECK_CLOCK){
        endtime = clock();
        cout<<"rearrange flow use time = "<<(double)(endtime - start)/CLOCKS_PER_SEC<<endl;
    }
    /****************** 输出结果 ******************/
    ofstream solution;
    ...
    bool flag = 1;
    for(auto it = site_90.begin(); it != site_90.end(); it++){
        if(flag == 1){
            solution << *it;
            flag = 0;
        }
