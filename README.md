...
## 成绩
### 初赛
在线训练：19(分数：442918，时间：285431ms)

正式赛：3(分数：2674532，时间：293463ms)
### 复赛
在线训练：16(分数：736375，时间：412928ms)

正式赛：14(分数：956280，时间：477638ms)
## 关于代码
(大家可以把建议更新在这里)
- 请大家务必将功能模块化，写成一个个函数
- 希望大家在该注释的地方写好注释
...
| 贪心2+平均4                   | 83376                | 36758                     | 加了塞满V的：6290602；没有塞满V的： | 没有塞V：987378（一次）                                      | 11.724                 |
| 贪心2+平均4（变为系数）       | 82789（base100）     |                           |                                     |                                                              |                        |
| 贪心2+平均4（变为系数+塞满V） | 72453（base100）     |                           | 4826864                             |                                                              |                        |
| 贪心2+平均5（使用代价）       | 80821（base100）     |                           |                                     | 没有塞V：918805（一次）                                      |                        |
| 贪心3+平均1                   | 95153（base100）     | 37191                     |                                     |                                                              | 8.682                  |
| 贪心3+平均2                   |                      |                           |                                     |                                                              |                        |
| 贪心3+平均3                   | 35701(base100)       |                           |                                     |                                                              |                        |
| 贪心3+平均4（系数）           | 85562(base100)       | 38672                     |                                     | 线上超时                                                     |                        |

塞满V之后好像后续push和pull比较难改进了？

几个需要思考的点：

1. 贪心完后5%的位置，前95%的位置是否需要塞满V？
2. 平均分的时候，使用站点带宽使用百分比应该会更合理一些，选取带宽使用百分比小的站点分配
3. 平均分的时候，是否需要将客户需求unroll，就是将所有客户的需求排一个序

一些问题：

1. 贪心3好还是贪心2好，是否有必要在贪心的时候对所有客户需求一起排序？测试（贪心2【不塞V】+平均4【系数】）与（贪心3【不塞V】+平均4【系数】）加一次push&pull，顺便可以跟（贪心1+平均1）对比
   - 贪心3【不塞V】+平均4【系数】+一次push&pull：（两次超时）（一次也超时）难道是给所有客户排序时间代价太大了？？
   - 贪心2【不塞V】+平均4【系数】+一次push&pull：987378
2. 用目前最好的方案73多（贪心是所有站点一起统计+代价系数平均+一次push+pull）得分：42min 超时
3. 用目前最好的方案73多（贪心是所有站点一起统计+代价系数平均+一次push+pull）塞V，得分：45min（不知道会不会超时）超时
4. 用目前最好的方案73多（贪心是所有站点一起统计+代价系数平均+一次pull）塞V：46min超时
5. 用目前最好的方案73多（贪心是所有站点一起统计+代价系数平均）塞V：51min超时
3. 贪心2+代价系数+一次pushpull得分：918805，说明用代价系数会更好一些

我们应该是希望对于同一个站点，它的所有的时间步（95%前）的值尽可能的一样（差别不大）
